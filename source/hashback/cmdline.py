...
@click.option("--log-unit-level", multiple=True,
              help="Set a program unit and its children to a different log level to the one defined by --log-level. Eg:"
                   " --log-unit-level=hashback.misc=WARNING")
...
@click.option("--read-every-byte", is_flag=True,
              help="Reduses network overhead at the expense of disk IO."
...
@click.option("--full-prescan", is_flag=True,
...
async def backup(description: Optional[str],
    ...
    try:
        if resume is not None:
            ...
            try:
                timestamp = datetime.now(settings.client_config.timezone)
...
async def _create_http_client(settings: Settings):
