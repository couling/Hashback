...
def _cached_server_session(client_id: UUID) -> protocol.ServerSession:
    return config.LOCAL_DATABASE.open_client_session(client_id=client_id)


def user_session(credentials=fastapi.Depends(AUTHENTICATOR)) -> protocol.ServerSession:
    session = _cached_server_session(client_id=security.get_client_id(credentials))
    return session


@asyncstdlib.lru_cache(maxsize=config.SERVER_SETTINGS.session_cache_size)
async def _cached_backup_session(client_id: UUID, backup_session_id: UUID) -> protocol.BackupSession:
    server_session = _cached_server_session(client_id=client_id)
...
async def backup_session(session_id: UUID, credentials=Depends(security.AUTHENTICATOR)):
    return await _cached_backup_session(client_id=security.get_client_id(credentials), backup_session_id=session_id)
